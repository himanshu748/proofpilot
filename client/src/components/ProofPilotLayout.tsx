...
import { BookOpenText, FileCheck2, LayoutDashboard, Menu, ShieldCheck, Sparkles, X } from "lucide-react";
...
export default function ProofPilotLayout({ children }: { children: ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#f7f8f5] text-slate-900">
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r border-slate-200/80 bg-[#fbfcfa] px-4 py-5 md:flex">
      ...
      {nav}
      <div className="mt-auto rounded-2xl border border-[#dce9e5] bg-[#f1f7f5] p-3.5"><div className="mb-1 flex items-center gap-2 text-xs font-semibold text-[#0b3b36]"><span className="size-1.5 rounded-full bg-emerald-500" />Nutrient DWS</div><p className="text-xs leading-5 text-slate-500">Extraction, review evidence, and finalization are traceable here.</p></div>
    </aside>
